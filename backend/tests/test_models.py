...
# from models import User, db as _db, Circle, Kpi, Kpi_Values
# from app import app
...
# def test_add_user(db, test_app_fixture):
#     with test_app_fixture.app_context():
#         user = User(email='test@example.com', user_login_name='testuser', first_name='Test', last_name='User', display_name='Test User')
#         db.session.add(user)
#         db.session.commit()
#         assert db.session.query(User).count() == 1
#         assert db.session.query(User).filter_by(id=user.id).first().is_gatekeeper == False

# def test_add_kpi(db, test_app_fixture):
#     with test_app_fixture.app_context():
#         circle=Circle(name='sales')
#         db.session.add(circle)
#         db.session.commit()

#         kpi = Kpi(circle_id=circle.id, name='gross sales', periodicity='monthly', unit='chf', initial_value=0, target_value=1000)
#         db.session.add(kpi)
#         db.session.commit()

#         circle_query = Circle.query.filter_by(id=circle.id).first()
#         assert circle_query.name == 'sales'
#         kpi_query = Kpi.query.filter_by(id=kpi.id).first()
