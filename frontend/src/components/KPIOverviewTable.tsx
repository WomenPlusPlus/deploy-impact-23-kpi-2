...
const KPIOverviewTable = () => {
  const [editKPI, setEditKPI] = useState<Record<string, any> | undefined>();
  const handleEditKPI = (id: string) => {
    setEditKPI(rows.find(item => item.id === id));
  };
    ...
    >
      ...
      >
        <Box
          ...
          pt={'12px'}
        >
          <IconButton onClick={() => setEditKPI(undefined)}>
            <CloseIcon />
          </IconButton>
        </Box>
        <Stack
        ...
        >
          ...
          >
            ...
            >
              ...
              <Typography sx={globalStyles.drawerLabelContent}>HR</Typography>
            </Stack>
            <Stack
              direction={'row'}
              columnGap={'24px'}
